...
const Home = () => {
    ...
    return (
        <div >
           ...
           <button className='bg-green-600 py-2 px-6 text-white font-bold rounded' type="submit">Live Demo</button>
            </div>
            <div className=' mx-8 ' >
            ...
            <h1 className='text-4xl font-semibold'>Customer Reviews({reviewItems.length})</h1>
                {
                   reviewItems.map(reviewItem =><ReviewItem key={reviewItem.id} reviewItem={reviewItem} ></ReviewItem> ) 
                }
                 <button className='bg-green-600 py-2 px-6 text-white font-bold rounded mb-20' type="submit">See All Reviews</button>
            </div>
        </div>
    );
};
